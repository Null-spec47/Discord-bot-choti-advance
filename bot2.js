...
async function safeReply(message, content) {
  try {
    ...
    console.log(`✅ Bot 2 Message sent: ${content.substring(0, 50)}... (with random word: ${randomWord})`);
    return sentMessage;
  } catch (error) {
    console.error('Bot 2 Message send error:', error.message);
    return null;
  }
}
...
client.on('ready', () => {
  ...
  console.log(`📱 Bot 2 is ready! Use commands in any channel.`);
...
client.on('messageCreate', async message => {
  ...
  console.log(`🎯 Bot 2 command: ${message.content}`);
    ...
    if (args.length >= 3) {
      ...
      try {
        ...
        if (channel) {
          ...
          safeReply(message, `✅ **MESSAGE REPEATED (BOT 2)** ✅\n**Channel:** ${channel.name}\n**Message:** ${messageToRepeat}\n**OWNS CHOTI ADVANCE 🔥**`);
          console.log(`✅ Bot 2: Message repeated to channel ${channelId}`);
        } else {
          safeReply(message, `❌ **CHANNEL NOT FOUND (BOT 2)** ❌\n**Channel ID:** ${channelId}\n**Error:** Channel not found\n**OWNS CHOTI ADVANCE 🔥**`);
        }
      } catch (error) {
        console.error(`❌ Bot 2: Failed to repeat message to channel ${channelId}:`, error.message);
        safeReply(message, `❌ **REPEAT FAILED (BOT 2)** ❌\n**Error:** ${error.message}\n**OWNS CHOTI ADVANCE 🔥**`);
      }
    } else {
      safeReply(message, `📖 **REPEAT HELP (BOT 2)** 📖\n**Usage:** !repeat [channel_id] [message]\n**Example:** !repeat 1234567890123456789 Hello everyone!\n**OWNS CHOTI ADVANCE 🔥**`);
    }
  }
  ...
  if (message.content.startsWith('!join')) {
    ...
    try {
      if (voiceChannel.type === 2 || voiceChannel.type === 'GUILD_VOICE') {
        ...
        safeReply(message, `✅ **BOT 2 JOINED** ✅\n**Channel:** ${voiceChannel.name}\n**ID:** ${vcId}\n**Status:** Bot 2 is now in voice channel\n**OWNS CHOTI ADVANCE 🔥**`);
      } else {
  ...
  if (message.content === '!vcstatus') {
    let status = `🎵 **VOICE CHANNEL STATUS (BOT 2)** 🎵\n\n`;
  ...
  if (message.content.startsWith('!volume')) {
    ...
    safeReply(message, `${volumeEmoji} **REAL VOLUME SET (BOT 2)** ${volumeEmoji}\n**Volume:** ${volumePercent}%\n**DB Level:** ${volumeDB.toFixed(1)} dB\n**Status:** REAL volume updated in REAL-TIME (NO RESTART)\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!stop') {
    if (player) {
      ...
      isLooping = false;
      soundQueue = [];
      safeReply(message, `⏹️ **STOPPED (BOT 2)** ⏹️\n**Status:** Audio stopped\n**OWNS CHOTI ADVANCE 🔥**`);
    } else {
  ...
  if (message.content === '!sounds') {
    try {
      ...
      safeReply(message, `🎵 **AVAILABLE SOUNDS (BOT 2)** 🎵\n**Sounds:** ${soundList}\n**Total:** ${files.length} sounds\n**OWNS CHOTI ADVANCE 🔥**`);
    } catch (error) {
  ...
  if (message.content === '!skip') {
    if (player && player.state.status !== AudioPlayerStatus.Idle) {
      player.stop();
      safeReply(message, '⏭️ **SKIPPED (BOT 2)** ⏭️\n**Status:** Current sound skipped\n**OWNS CHOTI ADVANCE 🔥**');
    } else {
      safeReply(message, '❌ **NO AUDIO (BOT 2)** ❌\n**Status:** No audio playing\n**OWNS CHOTI ADVANCE 🔥**');
    }
  }
  ...
  if (message.content === '!audioinfo') {
    ...
    safeReply(message, `🎵 **AUDIO INFO (BOT 2)** 🎵\n**Status:** ${status}\n**Current:** ${currentFile}\n**Volume:** ${volume}%\n**Loop:** ${loopStatus}\n**Queue:** ${queueSize} files\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!maxvolume') {
    ...
    safeReply(message, `🔊🔊🔊 **ULTRA MAX VOLUME SET (BOT 2)** 🔊🔊🔊\n**Volume:** 100000%\n**Status:** ULTRA MAXIMUM LOUDNESS ACTIVATED\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!superloud') {
    ...
    safeReply(message, `🔊🔊🔊🔊🔊 **ULTRA SUPER LOUD ACTIVATED (BOT 2)** 🔊🔊🔊🔊🔊\n**Volume:** 20000%\n**Status:** ULTRA ULTIMATE LOUDNESS\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!ultraloud') {
    ...
    safeReply(message, `🔊🔊🔊🔊🔊🔊🔊 **ULTRA LOUD ACTIVATED (BOT 2)** 🔊🔊🔊🔊🔊🔊🔊\n**Volume:** 500000%\n**Status:** ULTRA EXTREME LOUDNESS\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!megaloud') {
    ...
    safeReply(message, `🔊🔊🔊🔊🔊🔊🔊🔊🔊 **MEGA LOUD ACTIVATED (BOT 2)** 🔊🔊🔊🔊🔊🔊🔊🔊🔊\n**Volume:** 1000000%\n**Status:** MEGA EXTREME LOUDNESS\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!clearsound') {
    ...
    safeReply(message, `🔊🔊🔊🔊🔊🔊🔊🔊 **CLEAR SOUND ACTIVATED (BOT 2)** 🔊🔊🔊🔊🔊🔊🔊🔊\n**Volume:** 60000%\n**Status:** CRYSTAL CLEAR SOUND\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!ultraloudnorm') {
    ...
    safeReply(message, `🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊 **DISCORD BYPASS LOUDNORM (BOT 2)** 🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊\n**Volume:** 500000%\n**Status:** DISCORD LIMITS BYPASSED - ULTRA LOUDNORM\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!ultimateloud') {
    ...
    safeReply(message, `🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊 **ULTIMATE LOUD ACTIVATED (BOT 2)** 🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊\n**Volume:** 5000000%\n**Status:** ULTIMATE EXTREME LOUDNESS\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!dominatesound') {
    ...
    safeReply(message, `🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊 **DOMINATE SOUND ACTIVATED (BOT 2)** 🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊\n**Volume:** 10000000%\n**Status:** DOMINATE ALL SOUNDS\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!crystalclear') {
    ...
    safeReply(message, `💎💎💎💎💎💎💎💎 **CRYSTAL CLEAR SOUND ACTIVATED (BOT 2)** 💎💎💎💎💎💎💎💎\n**Volume:** 5000000%\n**Status:** CRYSTAL CLEAR AUDIO\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!puresound') {
    ...
    safeReply(message, `✨✨✨✨✨✨✨✨ **PURE SOUND ACTIVATED (BOT 2)** ✨✨✨✨✨✨✨✨\n**Volume:** 10000000%\n**Status:** PURE CRYSTAL AUDIO\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!extremeclear') {
    ...
    safeReply(message, `🔮🔮🔮🔮🔮🔮🔮🔮 **EXTREME CLEAR SOUND ACTIVATED (BOT 2)** 🔮🔮🔮🔮🔮🔮🔮🔮\n**Volume:** 20000000%\n**Status:** EXTREME CRYSTAL CLEAR\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!ultimateclear') {
    ...
    safeReply(message, `🌟🌟🌟🌟🌟🌟🌟🌟🌟🌟🌟🌟🌟🌟 **ULTIMATE CLEAR SOUND ACTIVATED (BOT 2)** 🌟🌟🌟🌟🌟🌟🌟🌟🌟🌟🌟🌟\n**Volume:** 50000000%\n**Status:** ULTIMATE CRYSTAL CLEAR\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content.startsWith('!pitch')) {
    ...
    safeReply(message, `🎵 **PITCH SHIFT SET (BOT 2)** 🎵\n**Pitch:** ${equalizerSettings.pitch} semitones\n**Range:** -12 to +12\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!voiceeffects') {
    ...
    safeReply(message, `🎛️ **VOICE EFFECTS STATUS (BOT 2)** 🎛️\n**Active Effects:** ${status}\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!resetvoice') {
    ...
    safeReply(message, `🔄 **VOICE EFFECTS RESET (BOT 2)** 🔄\n**Status:** All effects cleared\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!extremeloud') {
    ...
    safeReply(message, `🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊 **EXTREME LOUDNESS ACTIVATED (BOT 2)** 🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊\n**Volume:** 1000000%\n**Status:** EXTREME LOUDNESS - NO LIMITS\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!maximumloud') {
    ...
    safeReply(message, `🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊 **MAXIMUM LOUDNESS ACTIVATED (BOT 2)** 🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊🔊\n**Volume:** 10000000%\n**Status:** MAXIMUM LOUDNESS - ULTIMATE\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!list') {
    ...
    try {
      ...
      safeReply(message, `❌ **LIST FAILED (BOT 2)** ❌\n**Error:** Could not read files\n**OWNS CHOTI ADVANCE 🔥**`);
    }
  }
  ...
  if (message.content.startsWith('!delete')) {
    ...
    if (!fileName) {
      return safeReply(message, `❌ **DELETE FAILED (BOT 2)** ❌\n**Usage:** !delete [filename]\n**Example:** !delete sound1.mp3\n**OWNS CHOTI ADVANCE 🔥**`);
    }
    ...
    try {
      ...
      if (!fs.existsSync(filePath)) {
        return safeReply(message, `❌ **FILE NOT FOUND (BOT 2)** ❌\n**File:** ${fileName}\n**Status:** File does not exist\n**OWNS CHOTI ADVANCE 🔥**`);
      }

      fs.unlinkSync(filePath);
      safeReply(message, `🗑️ **FILE DELETED (BOT 2)** 🗑️\n**File:** ${fileName}\n**Status:** Successfully deleted\n**OWNS CHOTI ADVANCE 🔥**`);
    } catch (error) {
      console.error('Delete error:', error);
      safeReply(message, `❌ **DELETE FAILED (BOT 2)** ❌\n**Error:** Could not delete file\n**OWNS CHOTI ADVANCE 🔥**`);
    }
  }
  ...
  if (message.content === '!deleteall') {
    ...
    try {
      ...
      safeReply(message, `🗑️ **ALL FILES DELETED (BOT 2)** 🗑️\n**Deleted:** ${deletedCount}/${files.length} files\n**Status:** Successfully deleted\n**OWNS CHOTI ADVANCE 🔥**`);
    } catch (error) {
  ...
  if (message.content.startsWith('!purge')) {
    ...
    try {
      ...
      safeReply(message, `🗑️ **PURGED (BOT 2)** 🗑️\n**Deleted:** ${deletedCount} messages\n**OWNS CHOTI ADVANCE 🔥**`);
    } catch (error) {
      console.error('Purge error:', error);
      safeReply(message, `❌ **PURGE FAILED (BOT 2)** ❌\n**Error:** Could not delete messages\n**OWNS CHOTI ADVANCE 🔥**`);
    }
  }
  ...
  if (message.content === '!purgeall') {
    try {
      ...
      safeReply(message, `🗑️ **ALL PURGED (BOT 2)** 🗑️\n**Deleted:** ${deletedCount} messages\n**OWNS CHOTI ADVANCE 🔥**`);
    } catch (error) {
  ...
  if (message.content.startsWith('!status')) {
    ...
    if (!status || !['online', 'idle', 'dnd', 'invisible'].includes(status)) {
      return safeReply(message, `❌ **INVALID STATUS (BOT 2)** ❌\n**Valid:** online, idle, dnd, invisible\n**Usage:** !status [status]\n**OWNS CHOTI ADVANCE 🔥**`);
    }
    ...
    try {
      ...
      if (stealthStatus) {
        ...
        safeReply(message, `✅ **STEALTH STATUS UPDATED (BOT 2)** ✅\n**Status:** ${stealthStatus.status}\n**Activity:** ${stealthStatus.activity.name}\n**🕵️‍♂️ Stealth Mode: ACTIVE**\n**OWNS CHOTI ADVANCE 🔥**`);
      } else {
        client.user.setStatus(status);
        safeReply(message, `✅ **STATUS UPDATED (BOT 2)** ✅\n**Status:** ${status}\n**OWNS CHOTI ADVANCE 🔥**`);
      }
    } catch (error) {
      console.error('Status error:', error);
      safeReply(message, `❌ **STATUS FAILED (BOT 2)** ❌\n**Error:** Could not update status\n**OWNS CHOTI ADVANCE 🔥**`);
    }
  }
  ...
  if (message.content.startsWith('!activity')) {
    ...
    if (!['PLAYING', 'WATCHING', 'LISTENING', 'COMPETING'].includes(activityType)) {
      return safeReply(message, `❌ **INVALID TYPE (BOT 2)** ❌\n**Valid:** PLAYING, WATCHING, LISTENING, COMPETING\n**OWNS CHOTI ADVANCE 🔥**`);
    }
  ...
  if (message.content.startsWith('!changename')) {
    ...
    try {
      ...
      safeReply(message, `✅ **USERNAME CHANGED (BOT 2)** ✅\n**New Name:** ${newName}\n**Status:** Username updated successfully\n**OWNS CHOTI ADVANCE 🔥**`);
    } catch (error) {
  ...
  if (message.content === '!resetpfp') {
    try {
      ...
      safeReply(message, `✅ **PROFILE PICTURE RESET (BOT 2)** ✅\n**Status:** Avatar reset to default\n**OWNS CHOTI ADVANCE 🔥**`);
    } catch (error) {
  ...
  if (message.content === '!profile') {
    try {
      ...
      const profileInfo = `👤 **BOT 2 PROFILE INFO** 👤\n\n**Username:** ${username}\n**Tag:** ${tag}\n**ID:** ${id}\n**Created:** ${createdAt}\n**Avatar:** [Click Here](${avatarURL})\n\n**OWNS CHOTI ADVANCE 🔥**`;
  ...
  if (message.content.startsWith('!validate')) {
    ...
    try {
      const info = `✅ **VALIDATION RESULT (BOT 2)** ✅\n**Number:** ${phoneNumber}\n**Valid:** ✅ Yes\n**Format:** ✅ Correct\n**Country:** India\n**Type:** Mobile\n**OWNS CHOTI ADVANCE 🔥**`;
      safeReply(message, info);
    } catch (error) {
  ...
  if (message.content.startsWith('!addowner')) {
    ...
    saveOwners();
    safeReply(message, `✅ **OWNER ADDED (BOT 2)** ✅\n**User:** ${userId}\n**Status:** Access granted\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content.startsWith('!removeowner')) {
    ...
    saveOwners();
    safeReply(message, `❌ **OWNER REMOVED (BOT 2)** ❌\n**User:** ${cleanUserId}\n**Status:** Access revoked\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!equalizer') {
    ...
    safeReply(message, `🎛️ **EQUALIZER SETTINGS (BOT 2)** 🎛️\n**Current Settings:**\n${settings}\n**Usage:** !effect [type] [value]\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content === '!reseteq') {
    ...
    safeReply(message, `🔄 **EQUALIZER RESET (BOT 2)** 🔄\n**Status:** All effects reset to normal\n**Volume:** ${(currentVolume * 100).toFixed(0)}%\n**OWNS CHOTI ADVANCE 🔥**`);
  }
  ...
  if (message.content.startsWith('!uploadmp3')) {
    ...
    try {
      ...
      writer.on('finish', () => {
        safeReply(message, `✅ **FILE UPLOADED (BOT 2)** ✅\n**File:** ${fileName}\n**Size:** ${(attachment.size / 1024).toFixed(2)} KB\n**Status:** Successfully saved\n**OWNS CHOTI ADVANCE 🔥**`);
      });
  ...
  if (message.content === '!randomsound') {
    ...
    try {
      ...
      if (player) {
        ...
        player.play(resource);
        safeReply(message, `🎲 **RANDOM SOUND (BOT 2)** 🎲\n**Playing:** ${randomSound}\n**Status:** Random sound activated\n**OWNS CHOTI ADVANCE 🔥**`);
      } else {
  ...
  if (message.content === '!vcinfo') {
    ...
    try {
      ...
      if (voiceChannel) {
        ...
        const info = `📊 **VOICE CHANNEL INFO (BOT 2)** 📊\n**Channel:** ${voiceChannel.name}\n**ID:** ${voiceChannel.id}\n**Members:** ${memberCount}\n**Users:** ${userCount}\n**Bots:** ${botCount}\n**Status:** ${connection.state.status}\n**OWNS CHOTI ADVANCE 🔥**`;
        safeReply(message, info);
      } else {
  ...
  if (message.content === '!shuffle') {
    ...
    try {
      ...
      safeReply(message, `🔀 **QUEUE SHUFFLED (BOT 2)** 🔀\n**Queue Length:** ${soundQueue.length}\n**Status:** Queue order randomized\n**OWNS CHOTI ADVANCE 🔥**`);
    } catch (error) {
...
// Login bot 2
console.log('🛡️ Bot 2: Initializing secure connection... (2025 EDITION)');

// Check if token exists
if (!process.env.BOT2_TOKEN) {
  console.error('❌ **BOT TOKEN MISSING** ❌');
  console.error('**Error:** BOT2_TOKEN environment variable is not set');
  console.error('**Solution:** Create a .env file with your Discord bot token');
  console.error('**Example:** BOT2_TOKEN=your_discord_bot_token_here');
